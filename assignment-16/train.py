...
def get_ds(config):
    ...
    train_dataloader = DataLoader(train_ds, batch_size=config['batch_size'], shuffle=True, collate_fn=collete_fn)
...
def collete_fn(batch):
    ...
    return {
            ...
            "decoder_input": torch.vstack(decoder_inputs),  # (seq_len)
            "encoder_mask": torch.vstack(encoder_mask), # (1, 1, seq_len)
            "decoder_mask": torch.vstack(decoder_mask), # (1, seq_len) & (1, seq_len, seq_len),
            "label": torch.vstack(label),  # (seq_len)
            "src_text": src_text,
            "tgt_text": tgt_text,
        }
